...
def convert_models_to_onnx(json_file, nsga_net_path, logs_path, files_path):
    ...
    if os.path.exists(nsga_net_path):
        ...
        logging.warning(
            f"Path {nsga_net_path} does not exist. Ensure the path is correct."
        )

    logging.basicConfig(
        filename=os.path.join(
            os.makedirs(logs_path, exist_ok=True) or logs_path,
        ...
        try:
            ...
            if response.status_code == 200:
                model_file = os.path.join(
                    os.makedirs(files_path, exist_ok=True) or files_path,
                    f"{row.uid}.pt",
                )
            ...
            dummy_input = torch.randn(1, 3, 32, 32).to(
                "cpu"
            )  # Ensure the dummy input is on the CPU
            ...
            logging.info(
                f"Model {row.uid} has been converted to ONNX format at {onnx_path}"
            )
        except Exception as e:

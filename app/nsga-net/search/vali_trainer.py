...
class ValiTrainer:
        ...
        for epoch in range(self.epochs):
            ...
            for i, data in enumerate(self.trainloader, 0):
                ...
                if i % 100 == 99:  # Print every 100 mini-batches
                    ...
                    bt.logging.info(
                        f"Epoch [{epoch + 1}, {i + 1}] loss: {running_loss / 100:.3f}, accuracy: {accuracy:.2f}%"
                    )
                    running_loss = 0.0
                    correct = 0
                    total = 0
    ...
    def initialize_weights(self, model):
        ...
        for name, param in model.named_parameters():
            if (
                param.dim() >= 2
            ):  # Ensure the parameter has at least two dimensions
                if "weight" in name:
                    nn.init.kaiming_uniform_(param.data, a=math.sqrt(5))
            elif (
                param.dim() == 1
            ):  # Handle biases separately if they are one-dimensional
                if "bias" in name:
